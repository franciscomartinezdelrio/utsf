...
* `param`: it is a list with additional arguments for adjusting the behavior of the model. This parameter will not be used in this section and will be explained later.
...
Let us see an example in which a time series is forecast using the k-nearest neighbors regression model implemented in the FNN package:

```{r}
# Function to train the regression model
# In this case (KNN) just stores the training set
...
In this case, the `nnet()` function returns an S3 object of class `nnet`. Because the method `predict.nnet()` is already implemented we take advantage of it. Probably, the training set would need some preprocessing to obtain more accurace predictions.

Similarly, now we use a random forest model implemented in the `RandomForest` package (the random forest model directly supported by the **utsf** package is implemented in the `ranger` package):
...
The `randomForest()` function returns an S3 object of class `randomForest` with the trained model. The `predict.randomForest()` method is used as regression function.

# Setting the parameters of the regression models

Normally, a regression model can be adjusted using different parameters. By default, the models supported by our package are set using some specific parameters, usually the default values of the functions used to train the models (these functions are listed in a previous section). However, the user can set the parameters used to train a regression model with the `param` argument of the `create_model()` function. The `param` argument must be a named list with the names and values of the parameters to be set. Let us see an example:
...
The `param` argument is a named list with the names and values of the arguments of the model to be adjusted.

For example, in the next code chunk a K-nearest neighbor model is used. The function provided to train the model can be tuned with an argument named `k` with the number of nearest neighbors:
...
The list provided as argument for the parameter `param` in the `create_model()` function is passed as argument to the parameter `param` of the function that trains the model. In this case, in the second call to `create_model()` the argument of the parameter `param`, `list(k = 5)`, is passed as argument for the  parameter `param` of the `my_knn_model()` function.
