...
* `param`: it is a list with additional arguments for the model. This parameter will not be used in this section and it will be explained later.
...
As another example, we are going to use the neural network model implemented in the *nnet* package:
...
In this case, the `nnet()` function returns an S3 object of class `nnet`. Because the method `predict.nnet()` is already implemented we take advantage of it.

Similarly, now we use the random forest implementation of the `RandomForest` package:
...
The `randomForest()` function returns an S3 object of class `randomForest` and we rely on the `predict.randomForest()` method.

# Setting the parameters of the regression models

Normally, a regression model can be adjusted using different parameters. By default, the models supported by our package are set using some specific parameters, usually the default values of the functions used to train the models (these functions are listed in a previous section). However, the user can set the parameters used to train the regression models with the `param` argument of the `create_model()` function. The `param` argument must be a named list with the values of the parameters to be set. Let us see an example:
...
When the model to be trained is implemented by the user, the user can use the `param` parameter to receive arguments for tuning the model. The `param` argument is a named list with the values of the tuning arguments of the model.

For example, in the next code a K-nearest neighbor model is implemented. The function used to train the model can receive an argument named `k` with the number of nearest neighbors:
...
As another example, suppose that we create a random forest model using the functionality in the *randomForest* package. By default, we will use the default parameters of the `randomForest::randomForest()` function to build the model, except for the number of trees (it will be 200). However, the user can tune the model with other values:
...
In this case, no preprocessing is applied (`trend = "none"`). It can be observed how the forecast does not capture the trending behavior of the series because regression tree models predict averaged values of the training targets.
...
```{r}
timeS <- ts(c(1, 3, 7, 9, 10, 12))
m <- create_model(timeS, lags = 1:2, trend = "additive")
cbind(m$features, Targets = m$targets)
```
...
In this last code snippet we have used the default value of `trend`, because it applies the additive transformation.

You can only transform the targets, so that the features are not transformed:
...
However, our experience tells us that transforming the features tends to improve forecast accuracy. The point of transforming the features is to remove the effects of the level of the series in the feature space.
...
* `transform_features`: Its default value is `TRUE`, so that if a series is transform with the additive or multiplicative transformation both its features and targets are transformed.
